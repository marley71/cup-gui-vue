...
function resetPassword() {
  error.value = false;
  var hashParams = CrudHelpers.getHashParams();
  console.log('reset password', email.value, hashParams)


  cs.Server.post('/api/reset-password', {
    email: email.value,
    ...
    CrudCore.alertSuccess(json.msg);
...
<template>
  <div class="bg-surface-50 dark:bg-surface-950 px-6 py-20 md:px-20 lg:px-80">
    <div
        class="bg-surface-0 dark:bg-surface-900 p-8 md:p-12 shadow-sm rounded-2xl w-full max-w-[48rem] mx-auto flex flex-col gap-8">
      ...
      <div class="flex flex-col items-center gap-4">
        ...
        <div class="flex flex-col gap-6 w-full">
          <div v-if="error" class="gap-2 w-full text-red-500">
            <div>{{ errorMsg }}</div>
          </div>
          <div class="flex flex-col gap-6 w-full">
            <div class="pt-4 guest-sign-in text-xl text-left">Inserisci i dati per effettuare il reset della tua
              password.
            </div>
            <IconField>
              <InputIcon class="fa fa-envelope "/>
              <InputText v-model="email" type="text" class="w-full px-3 py-2 shadow-sm rounded-lg" placeholder="E-mail"/>
            </IconField>
            ...
            <Button label="Reset Password" icon="fa fa-envelope" class="w-full py-2 rounded-lg flex justify-center items-center gap-2"
                    @click="resetPassword"
            >
              <template #icon>
                <i class="far fa-envelope text-base! leading-normal!"/>
              </template>
            </Button>
...
//background: linear-gradient(rgba(255,255,255,.9), rgba(255,255,255,.9)), url("/layout/images/login-bg.png") no-repeat; //background-size: cover;
}
</style>
