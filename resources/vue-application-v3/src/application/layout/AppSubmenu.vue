...
export default {
    ...
    mounted() {
        ...
        EventBus.on('menuitem-click', () => {
            console.debug('clicche');
        });
    },
    methods: {
        btoa(str) {
        ...
        onMenuItemClick(event, item, index) {
            console.debug('clicche2');
            let rowToMove = document.getElementById('menu_' + item.to);
            window.MENUCLICCHED = item.to;
            // if (rowToMove) {
            //     setTimeout(function () {
            //         console.debug('clicche3',rowToMove);
            //         rowToMove.scrollIntoView({ behavior: 'smooth' });
            //     },2000)
            // }
            console.debug('clicche2');
            if (item.disabled) {
