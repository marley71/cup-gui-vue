...
<!--            <img id="topbar-logo" :src="logoImage" alt="cupparis-vue" />-->
            <img id="topbar-logo" src="@/application/assets/milano.jpg" topBarApplication>
        </router-link>
...
//import Utility from "@rome/service/Utility";
import Utility from "@/application/services/Utility";
...
export default {
    ...
    methods: {
        ...
        async setUserInfo() {
            let that = this;
            nextTick();
            console.debug('user',that.user,'userInfo',that.userInfo);
            that.user.setUserInfo(that.userInfo);
        }
    },
    data() {
      let topbarConfig = {
      ...
      for (let k in templateConfig.topbarConfig) {
        topbarConfig.boolean[k] = templateConfig.topbarConfig[k]
      }
        return {
            config : topbarConfig,
            userInfo : {},
            logoImage :null,
            user : {},
            //devString : import.meta.env.VITE_MODE
        }
    },
    mounted() {
        let that = this;
